...
public class RealityListViewModel : ViewModelBase
{
    private readonly ISeriesService _seriesService;

    private IReadOnlyList<Series> _realityShows = [];
    public IReadOnlyList<Series> RealityShows
    {
        get => _realityShows;
        private set => SetProperty(ref _realityShows, value);
    }

    private IReadOnlyList<Series> _filteredShows = [];
    public IReadOnlyList<Series> FilteredShows
    {
        get => _filteredShows;
        private set => SetProperty(ref _filteredShows, value);
    }
    ...
    public IReadOnlyList<string> AvailableStatuses { get; } = ["All", "Ongoing", "Ended"];

    public int TotalSeasons => _realityShows.Sum(s => s.Seasons);

    public RealityListViewModel(ISeriesService seriesService)
    {
    ...
    public async Task LoadAsync()
    {
        ...
        try
        {
            RealityShows = _seriesService.GetRealityShows();

            var genres = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "All" };
            foreach (var item in RealityShows)
            {
        ...
        {
            ErrorMessage = "Failed to load reality show archive.";
        }
        finally
        {
            IsLoading = false;
    ...
    {
        var result = _realityShows.AsEnumerable();
